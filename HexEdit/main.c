 ...
 * @LastEditTime: 2026-04-14 18:08:22
...
// 自动识别 8/10/16 进制字符串 → 无符号整数
static uint32_t auto_str_to_uint32(const char *str)
{
    return (uint32_t)strtoul(str, NULL, 0);
}

// 自动识别 8/10/16 进制字符串 → 有符号整数
static int32_t auto_str_to_int32(const char *str)
{
...
int main(int argc, char **argv)
{
    ...
    int inFileNum = 0;
    int inFileType[64];
    uint32_t inbinaryAddress[64];
    static char inFilePath[64][512];

    int outFileType = 0;
    int binaryOutMode = 0xFF;
    char outFilePath[1024] = "";

    uint32_t binaryAddress = UINT32_MAX;
    int currentFileType = -1;
    ...
    while ((c = getopt_long(argc, argv, "f:o:bhx:i:v", long_options, &option_index)) != -1)
    {
        ...
        default:
            ...
            printf("\t-i 设置输出二进制文件的充填字节 eg:-i 0xFF(设置充填字节为0xFF,默认0xFF); -i 0xFFFFFFFF 会分段输出\n");
    ...
    else // del set cut
    {
        ...
        while (temp)
        {
            if (i == 0)
            {
                address = auto_str_to_uint32(temp);
            }
            else if (i == 1)
            {
                len = auto_str_to_uint32(temp);
            }
            else if (i == 2)
            {
                value = auto_str_to_uint32(temp);
            }
            i++;
            temp = strtok(NULL, ":");
        }
            ...
            cmd == 203 && i == 2)
        {
            ...
            if (inFileNum == 1)
            {
                ...
                else if (cmd == 202)
                {
                    ...
                    memset(_temp_buf, (int)value, len);
